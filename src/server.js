...
app.get("/api/dashboard", async (_req,res) => { const latest=await store.latest(), settings=await runtimeSettings(); const since=new Date(Date.now()-86400000).toISOString(); res.json({ latest, history:await store.history(since), freshness:dataFreshness(latest?.timestamp,new Date(),settings.staleMinutes,settings.offlineMinutes), alert:await store.alertState(), settings, timeZone:env.TIME_ZONE||"Asia/Makassar", pushAvailable:Boolean(env.VAPID_PUBLIC_KEY) }); });
